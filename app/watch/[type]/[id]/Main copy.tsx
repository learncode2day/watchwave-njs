...
radial-gradient(at 0% 0%, hsla(343,100%,76%,1) 0px, transparent 50%);`;
  ...
  if (isInFuture || isinDMCA) {
    return (
      <Fallbacks isInFuture={isInFuture} isinDMCA={isinDMCA} result={result} />
    );
  }
          ...
          {result && (
            <>
              {result?.media_type === "tv" &&
                ...
                playerVisible && (
                  <Player
                    setPlayerVisibility={playerV}
                    sourceCollection={sourceCollectionTV}
                    result={result}
                    sources={sources}
                  />
                )}
              {result?.media_type === "movie" && playerVisible && (
                <Player
                  setPlayerVisibility={playerV}
                  sourceCollection={sourceCollectionMovie}
                  result={result}
                  sources={sources}
                />
              )}
