...
const NextEpisode = () => {
	const { nextEpisodeVisible, setNextEpisodeVisible, setEpisode, episode, season, setSeason } = useTVStore((state) => state);
	const { setPlayed } = usePlayerStore((state) => state);
	...
	const handleNextEpisode = async () => {
		...
		if (episode === result.seasons[season].episodes.length) {
			if (season !== result.seasons.length - 1) {
				await set(season + 1, 1, result?.id);
				setSeason(season + 1);
				setEpisode(1);
				setPlayed(0);
			} else {
			...
			await set(season, episode + 1, result?.id);
	...
	return (
		...
		>
			<div className="fr pointer-events-auto absolute bottom-24 right-5 z-20 gap-4 light">
				{!final ? (
					<>
						<Button size="md" radius="sm" className="group h-11 font-semibold" onClick={handleNextEpisode}>
							Next Episode
							<IoPlayForward size={16} className="text-sm transition-transform duration-500 group-hover:scale-110 sm:text-base" />
						</Button>
						<Button
							size="md"
							radius="sm"
					...
					<Button size="md" radius="sm" className="group h-11 bg-transparent font-semibold" onClick={() => setNextEpisodeVisible(false)}>
						Close
					</Button>
				)}
			</div>
