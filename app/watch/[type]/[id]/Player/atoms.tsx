...
export const Rewind = ({
  ...
  seek15s: (direction: "forward" | "backward") => void;
}) => {
  return (
...
export const Volume = ({
  ...
  changeVolume: (value: number | number[]) => void;
}) => {
  const { volume, setVolumeVisible, volumeVisible, muted, setMuted } =
          ...
          >
            <Slider
              ...
              classNames={{
                ...
                thumb:
                  "w-4 h-4 after:bg-white after:w-4 after:h-4 before:w-4 before:h-4",
...
export const Time = () => {
  const { played, duration, timedetails, setTimedetails, timeLeft } =
    usePlayerStore((state) => state);
  return (
        ...
        {!timedetails ? (
          <div className="fr gap-2">
            <p suppressHydrationWarning>
              {new Date(played * 1000).toISOString().substring(11, 19)}
            </p>
            <p>/</p>
            <p suppressHydrationWarning>
          ...
          <div className="fr gap-2">
            ...
            <p suppressHydrationWarning>
              Finishes at{" "}
              {timeLeft &&
                format(new Date(Date.now() + timeLeft * 1000), "hh:mm a")}
            </p>
          </div>
        )}
      </div>
...
export const Airplay = () => {
  ...
  return (
    <div
      className="atom"
      onClick={() => alert("Airplay not supported on this device")}
    ></div>
  );
};

export const Subtitle = () => {
  const { setSubtitlesPanelVisible, subtitlePanelVisible } = useSubtitlesStore(
    (state) => state,
  );
  return (
    <div
      className="atom"
      onClick={() => setSubtitlesPanelVisible(!subtitlePanelVisible)}
    >
      <PiSubtitles />
    </div>
  );
...
export const Episodes = () => {
  const { episodePanelVisible, setEpisodePanelVisible } = useTVStore(
    (state) => state,
  );
  return (
    <div
