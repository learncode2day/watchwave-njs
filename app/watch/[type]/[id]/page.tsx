...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      ...
      images: [
        image && {
          url: `/api/og?img=${image}&title=${content.title || content.name}`,
        },
      ],
    },
  };
...
export default async function Page({ params }: Props) {
  ...
  const movieWeb = async () => {
    ...
    const content: MovieDetails | ShowDetails = await fetch(
      `https://api.themoviedb.org/3/${type}/${id}?language=en-US`,
      options,
    ).then((res) => res.json());
  ...
  const fetchTV = async () => {
    try {
      ...
      // if the first object in the array has a name of "Specials", move it to the end
      if (tempContent[0].name === "Specials") {
        tempContent.push(tempContent.shift());
      }
  ...
  async function fetchSections() {
    ...
    cred = cred.cast.filter(
      (cast: castProps) => cast.known_for_department === "Acting",
    );
