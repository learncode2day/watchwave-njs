...
const Search: React.FC = () => {
	...
	const debouncedSearchTerm = useDebounce(fieldQuery, 300);
	...
	useEffect(() => {
		setIsLoading(true);
		fetch(`https://api.themoviedb.org/3/search/multi?query=${fieldQuery}&include_adult=false&language=en-US&page=${page}`, options)
			.then((res) => res.json())
			.then((data) => {
	...
	useEffect(() => {
		...
		console.log(debouncedSearchTerm, 'searching...');
		setIsLoading(true);
		fetch(`https://api.themoviedb.org/3/search/multi?query=${debouncedSearchTerm}&include_adult=false&language=en-US&page=${page}`, options)
			.then((res) => res.json())
			.then((data) => {
		...
		<div className="relative min-h-screen w-full bg-neutral-950">
			...
			<div className="fc mx-auto max-w-6xl justify-start px-3 pt-24 antialiased sm:px-10 sm:pl-36">
				...
				<div className="fc w-full gap-2">
					...
					/>
					<p className="text-sm font-semibold text-zinc-400">Enter exact title to search</p>
				</div>
				<SearchResults
					isLoading={isLoading}
