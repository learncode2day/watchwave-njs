...
const Loading = () => {
	...
	// 		const randomIndex: number = Math.floor(Math.random() * loadingMessages.length);
			...
			{/* <AnimatePresence>
				{message.length !== 0 &&
					message.split('').map((letter, index) => (
						<motion.span initial="hidden" animate="visible" exit="exit" variants={child} key={message + index}>
							{letter === ' ' ? '\u00A0' : letter}
						</motion.span>
					))}
			</AnimatePresence> */}
