...
const Details = () => {
	const { setPlayerVisible, result, sections } = useMainStore((state) => state);
	...
	const { recommendations, credits, keywords, videos, reviews, external } = sections || {}; // Add default empty object to handle null case
	...
	return (
		<div>
			...
			{result && (
				<div className="fc w-full pt-7  mt-[25vh] z-10 relative">
					<div className="fc md:fr mb-12 w-full max-w-7xl gap-5 px-5 sm:px-10 md:items-start md:justify-start">
						...
						<div className="fc items-start gap-2">
							...
							<div className="fr gap-2 light">
								<PlayButton action={() => setPlayerVisible(true)} />
								{value2 && <WatchlistButton content={result} isInWatchlist={value2[result.media_type].includes(result.id)} />}
							</div>
							<p className="max-w-[50ch] text-sm sm:text-lg">{result.overview}</p>
							<ul className="fc gap-4 lg:fr items-start justify-start lg:justify-start w-full flex-wrap">
